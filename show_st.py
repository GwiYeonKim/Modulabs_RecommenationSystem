...
def load_data():
    ...
    #ValueError: Only input tensors may be passed as positional arguments. The following argument value should be passed as a keyword argument: 0 (of type <class 'int'>) 수정
    #model([[0 for _ in range(len(field_dims))]])

    dummy_input = tf.constant([[0] * len(field_dims)], dtype=tf.int32) 
    model(dummy_input)
...
def get_recom(user, user_non_seen_dict, user_df, movies_df, r_year, r_month, model, label_encoders):
    ...
    input_tensor = tf.convert_to_tensor(merge_data.values, dtype=tf.int32)
    recom_top = model(input_tensor)  # model.predict(...) 대신 직접 호출
    ...
    # 각 영화에 대한 예측 점수 (float)
    scores = tf.squeeze(recom_top).numpy()  # shape: (N,)

    # 점수 기준으로 상위 N개 영화의 인덱스 추출
    top_k = 10  # 예: 10개 추천
    top_k_indices = np.argsort(scores)[::-1][:top_k]

    # 해당 인덱스를 이용해 movie_id 라벨 가져오기
    movie_ids_encoded = merge_data['movie_id'].iloc[top_k_indices].values
    origin_m_id = label_encoders['movie_id'].inverse_transform(movie_ids_encoded)
